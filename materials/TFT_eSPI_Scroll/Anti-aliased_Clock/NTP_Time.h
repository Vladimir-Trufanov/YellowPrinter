...
// Petrozavodsk
/*
 TimeChangeRule — это объект (или структура данных), который описывает, когда в каком-то конкретном регионе происходит смена времени:
 переход на летнее (дневное) время или обратно на стандартное. Проще говоря: это «инструкция», которая говорит программе, в какой момент 
 (день, час) и относительно какого смещения от UTC (всемирного координированного времени) меняется часовой пояс. 
 
 Как это работает на практике

 Обычно для одного часового пояса задают два таких правила: одно — для начала летнего времени, 
 другое — для начала стандартного. Например, для Восточного часового пояса США:
 летнее время (EDT) начинается во второе воскресенье марта в 02:00 по местному времени;
 стандартное время (EST) начинается в первое воскресенье ноября в 02:00 по местному времени.

 В коде это часто записывают так (на примере библиотеки для Arduino Timezone):
 TimeChangeRule usEDT = {"EDT", Second, Sun, Mar, 2, -240}; // UTC - 4 часа
 TimeChangeRule usEST = {"EST", First, Sun, Nov, 2, -300};  // UTC - 5 часов
 Здесь:
 Second, Sun — вторая воскресенье марта;
 2 — час;
 -240 (и -300) — смещение в минутах (от UTC);
 "EDT", "EST" — сокращения названий часовых поясов.
 
 Важные нюансы
 В некоторых локациях бывают особенности. Например, в Западной Гренландии из-за комбинации правила «последнее» 
 и раннего момента смены времени переход может происходить поздно вечером предыдущего дня. 
 В таких случаях для часа используют отрицательное значение (например, -1 означает 23:00). 
...
void decodeNTP(void)
{
  ...
  while (millis() < waitTime && !timeValid)
  {
    ...
    if (udp.parsePacket())
    {
      ...
      uint8_t hh = hour(utc)+3;  // Петрозаводское время
